...
pub fn install_kernel_binaries(
    ...
    if !src_kernel.exists() {
        return Err(format!("Kernel source file does not exist: {:?}", src_kernel));
    }

    let kernel_name = src_kernel
        .file_name()
        .ok_or("Invalid kernel filename")?;
    ...
    if let Some(ref initrd_path) = artifacts.initramfs_path {
        ...
        if src_initrd.exists() {
            let initrd_name = src_initrd
                .file_name()
                .ok_or("Invalid initramfs filename")?;
            let dest_initrd = boot_dir.join(initrd_name);

            fs::copy(src_initrd, &dest_initrd).map_err(|e| {
                format!("Failed to copy initramfs to {:?}: {}", dest_initrd, e)
            })?;
        }
    }
