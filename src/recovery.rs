...
pub fn trigger_emergency_cleanup(target_mount: &Path, target_disk: Option<&Path>) {
    ...
    if let Some(disk) = target_disk {
        warn!("Wiping partition table on {:?} to prevent corrupted boot state...", disk);
        ...
        match zap_status {
            Ok(status) if status.success() => info!("Successfully wiped partition table on {:?}", disk),
            Ok(status) => error!("sgdisk returned non-zero status during rollback: {}", status),
